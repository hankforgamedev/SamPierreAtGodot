...
**Hypothesis:** `Label.position` is the top-left corner; map characters are also top-left. The train label's own height is not subtracted, so the train sits _below_ the intended row rather than _on_ it. Or `_base_map_pos` doesn't account for the map's top margin inside the Control.
...
- Line 118: `_advance()` — appends completed entry to `_log_bbcode` at line 188, _then_ calls `_update_display()`. Check whether `_log_bbcode +=` fires before or after the display call.
